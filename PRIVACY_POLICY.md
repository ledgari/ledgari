**Privacy Policy**

*Last updated: 30/10/2025*

This Privacy Policy explains how **Ledgari** (“the App”, “we”, “our”, or “us”) collects, uses, and protects your information.

---

### 1. Information We Collect

We collect the following personal information when you create an account or use the App:

* Full name
* Business name
* Email address
* Phone number (optional)
* Preferred Currency

We also store data you enter into the App, such as transactions, customers, sales, and expenses.

---

### 2. How We Use Your Information

Your information is used only to:

* Create and manage your account
* Provide record-keeping features
* Sync your data across devices
* Enable subscription features and billing

We do **not** sell, rent, or share your personal information for advertising or marketing purposes.

---

### 3. Data Storage and Security

* Your data is securely stored on our servers using **Supabase**.
* A local copy of your data may be stored on your device to enable offline access.
* We take reasonable measures to protect your information from unauthorized access or disclosure.

---

### 4. Third-Party Services

We use the following third-party services:

* **Supabase** — for authentication and data storage
* **Google Play Billing** — to process subscriptions

These services only access data necessary to perform their functions and are governed by their own privacy policies.

---

### 5. Data Sharing

We do not share your personal data with third parties except where required to provide core app functionality or comply with legal obligations.

---

### 6. Data Retention

Your data is retained for as long as your account remains active.
You may request deletion of your account and associated data at any time.

---

### 7. Your Rights

You have the right to:

* Access your personal data
* Update or correct your information
* Request deletion of your account and data

To request account and data deletion, users may contact us via email at ledgariapp@gmail.com. Requests should include the account email address and business name for identification.

---

### 8. Children’s Privacy

This App is not intended for use by children under the age of 13, and we do not knowingly collect personal information from children.

---

### 9. Changes to This Policy

We may update this Privacy Policy from time to time. Changes will be reflected on this page.

---

### 10. Contact Us

If you have any questions about this Privacy Policy, please contact us at:

**Email:** ledgariapp@gmail.com

---

**By using the App, you agree to this Privacy Policy.**
